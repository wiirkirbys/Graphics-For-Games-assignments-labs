...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	...
	float3 bumpNormal = N * UVW.z + normalTex.x * T + normalTex.y * B;
	float4 diffuse = DiffuseColor * DiffuseIntensity * max(0, dot(bumpNormal, L));
	float4 specular = SpecularColor * SpecularIntensity * pow(saturate(dot(H, normalize(bumpNormal))), Shininess);
